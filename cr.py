...
# --- Gauge chart function (retained for visualization reference, not used in main display) ---
def create_gauge(value, title, steps=None):
    """Creates a Plotly gauge chart for metric visualization."""
    PASTEL_COLORS = {'red': '#ff6961', 'orange': '#ffb347', 'green': '#77dd77'}
    ...
    fig = go.Figure(go.Indicator(
        mode="gauge+number", value=value, title={'text': title}, gauge=gauge_config,
        number={'suffix': '%'}
    ))
...
benchmark_view = st.sidebar.radio(
    ...
    index=0,
    horizontal=False,
    help="Select the benchmark to compare against (e.g., 'Total Loss vs. Optimal' vs. 'Loss vs. Target')."
)
