...
def calculate_capacity_risk(_df_raw, toggle_filter, default_cavities, target_output_perc):
    ...
    for date, daily_df in df_production_only.groupby('date'):
        ...
        if df_valid.empty or len(daily_df) < 2:
            ...
            # Ensure required time columns are initialized to 0 for aggregation
            results['Filtered Run Time (sec)'] = 0
            results['Actual Cycle Time Total (sec)'] = 0
            results['Capacity Loss (downtime) (sec)'] = 0
            results['Actual Output (parts)'] = 0
            results['Optimal Output (parts)'] = 0
            results['Capacity Loss (downtime) (parts)'] = 0
            results['Capacity Loss (slow cycle time) (parts)'] = 0
            results['Capacity Gain (fast cycle time) (parts)'] = 0
            results['Total Capacity Loss (parts)'] = 0
            results['Total Capacity Loss (sec)'] = 0
            results['Target Output (parts)'] = 0
            results['Capacity Loss (vs Target) (parts)'] = 0
    ...
    if df_raw is not None:
        ...
        with st.spinner("Calculating Capacity Risk..."):
            ...
            if results_df is not None and not results_df.empty:
                ...
                # Correct Run Time Calculation: Sum of all daily Filtered Run Times
                ...
                # 2. Actual vs Target/Optimal Output
                ...
                with col2:
                    st.metric("Target Output (parts)", f"{total_target:,.0f} ({target_output_perc_overall:.1%})")
                    st.metric(f"Actual Output (parts) ({actual_output_perc:.1%})", f"{total_produced:,.0f}")

                # 3. Total Capacity Loss (vs Benchmark)
                ...
                # --- Collapsible Daily Summary Table (vs Benchmark) ---
                ...
                # --- All-Time Summary Chart (Always Shows Physical Build-up vs Benchmark) ---
                st.header("Production Output Overview (parts)")
                
                # Dynamic Setup for the 4th Bar
                if benchmark_view == 'Optimal Output':
                    final_bar_label = 'Optimal Output (parts)'
                    benchmark_value = total_optimal
                    benchmark_text = "Optimal"
                else:
                ...
                fig_summary.add_trace(go.Bar(
                    x=[categories[3]], y=[benchmark_value], name=final_bar_label,
                    marker_color='grey', text=[f"{benchmark_value:,.0f}<br>{benchmark_text}"],
